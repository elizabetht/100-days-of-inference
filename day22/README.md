...
A production vLLM Dockerfile builds from `nvcr.io/nvidia/cuda:12.4.1-cudnn-devel-ubuntu22.04`, pins vLLM and FastAPI versions, and either bakes weights into the image or mounts them from a volume at runtime. The NVIDIA Container Toolkit passes GPU access into the container without exposing the host driver. The container starts, loads weights into GPU High-Bandwidth Memory (HBM), and begins serving on the OpenAI-compatible port. A `HEALTHCHECK` directive with a 120-second `start-period` gives the model time to load before the orchestrator marks the container unhealthy.
