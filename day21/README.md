...
The GPU count on an instance determines which models fit and what parallelism is available. A single H100 (80 GB of High-Bandwidth Memory, HBM) handles models up to ~40B parameters in FP16 with comfortable Key-Value (KV) cache headroom. Four GPUs (320 GB) covers 70B in FP16. Eight GPUs (640 GB) can serve 405B. GB200 NVL72, with 13.5 TB across 72 Blackwell GPUs, handles any open model available today, and the forthcoming Vera Rubin NVL72 lifts that ceiling further with 288 GB of HBM4 per GPU.
